...
// synchronous publishing and wildcard subscription. The handlers
// registered with [Bus.Subscribe] are run in their own goroutine and
// all events are processed in the order they are published. Each
// handler has its own event queue which is used to store events. When
// an asynchronous event is published on the bus and the event can't
// be stored in the event queue of a handler because its queue is
// full, the event is dropped for this handler and a [Dropped] event
// is generated by the bus. A slow handler doesn't impact other
// handlers when publishing asynchronous events.
package eventbus
